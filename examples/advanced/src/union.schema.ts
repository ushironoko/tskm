...
// set as data: `.discriminant`, `.literals`, and `.mapping` (a tag -> member map). A registry or
// exhaustive `switch` can be DERIVED from the schema instead of re-declared by hand (see main.ts).
//
// `tskm gen` materializes the whole union into a concrete `Shape` type, with no `Infer<typeof
// shapeSchema>` paid at the use site. The generated type is the same union a plain
