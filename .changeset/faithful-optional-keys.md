...
The mode is opt-in and travels with the schema (no global state). It is off by default, so existing `object()` schemas keep the current `k: T | undefined` type, the `undefined`-materializing runtime output, and the current generated type, byte-for-byte. The option is passed as a non-positional object, so it does not collide with the existing trailing `message` argument; `object(entries, "message")` is unchanged. `ObjectSchema` and `InferObjectOutput` gain a defaulted type parameter for the mode.
