...
- Types: `RecordSchema` is parameterized over the key schema, so the inferred key type is `InferOutput<TKey>` instead of `string`. A `templateLiteral` key yields a templated index signature, a `picklist` key yields a finite literal key set.
