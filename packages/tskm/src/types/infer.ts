...
export type InferInput<TSchema extends AnySchema> = NonNullable<TSchema["~types"]>["input"]

/** Infers the output type of a schema (the type the AOT compiler materializes). */
export type InferOutput<TSchema extends AnySchema> = NonNullable<TSchema["~types"]>["output"]
