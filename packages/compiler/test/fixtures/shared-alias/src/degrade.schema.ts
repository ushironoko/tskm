import { fallback, type Infer, number, object, parse, pipe, string, transform } from "@tskm/core"
...
// A `fallback()` schema: the walker has no case for `type: "fallback"`, so it falls back to a
// bare `unknown`. Under the flag the resolver must skip it and let the CHECKER type (string)
// stand, NOT overwrite the correct type with `unknown` (issue #22 review).
export const safeNameSchema = fallback(string(), "anon")
